...
{
    const Mesh m = MeshBuilder(unit_cube()).build();
    ASSERT_EQ( m.facets.size() , m.orientedEdgesPerFacet.size() ); // The tables per facet have consistent size
    for(size_t facet_index=0 ; facet_index < m.facets.size() ; ++facet_index) {
        Facet f = m.facets.at(facet_index);
        ASSERT_EQ( f.vertex_index.size() , m.orientedEdgesPerFacet.at(facet_index).size() ); // the facet has as many edges as vertices
        for(size_t iv=0 ; iv < f.vertex_index.size() ; ++iv ) {
            ASSERT_EQ( f.vertex_index[iv] ,
                       m.first_vertex_of_oriented_edge( m.orientedEdgesPerFacet.at(facet_index).at(iv) ));
            size_t iv_next = (iv+1) % f.vertex_index.size();
            ASSERT_EQ( f.vertex_index[iv_next] ,
                       m.second_vertex_of_oriented_edge( m.orientedEdgesPerFacet.at(facet_index).at(iv) ));
        }
    }
}
...
{
    const Mesh m = MeshBuilder(unit_cube()).build();
    ASSERT_EQ(m.edges.size() , m.facetsPerEdge.size()); // The tables per edge have consistent size
    for(size_t edge_index=0 ; edge_index < m.facetsPerEdge.size() ; ++edge_index) {
        for(size_t fi=0 ; fi < m.facetsPerEdge.at(edge_index).size() ; ++fi ) {
            ...
            auto oriented_edges_of_this_facet = m.orientedEdgesPerFacet.at( facet_index );
    ...
    for(size_t facet_index=0 ; facet_index < m.orientedEdgesPerFacet.size() ; ++facet_index) {
        for(size_t ie=0 ; ie < m.orientedEdgesPerFacet.at(facet_index).size() ; ++ie ) {
            size_t edge_index = Mesh::get_oriented_edge_index(m.orientedEdgesPerFacet.at(facet_index).at(ie));
