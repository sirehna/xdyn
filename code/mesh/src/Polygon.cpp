...
area(mesh->facets.at(facet_idx).area), unit_normal(mesh->facets.at(facet_idx).unit_normal)
{
}

Polygon::Polygon(const Matrix3x& matrix //!< Mesh given as a matrix (one point per column, all columns are used)
                ) : mesh(MeshPtr(new Mesh(MeshBuilder(matrix).build()))), facet_idx(0), area(::area(matrix)), unit_normal(::unit_normal(matrix))
{
}
