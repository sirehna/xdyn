...
HOSComs::Params* get_params();
HOSComs::Params* get_params()
{
    HOSComs::Params* params = new HOSComs::Params();
    // Grid parameters
    params->set_n1(128); // Number of modes per knot on axis X
    params->set_n2(32); // Number of modes per knot on axis Y
    params->set_m(3); // Order of the HOS method (order of the Taylor expansion of the velocity potential & the W velocity
    params->set_p1(1); // Anti-aliasing parameter for axis X
    params->set_p2(1); // Anti-aliasing parameter for axis Y
    ...
    params->set_toler(1E-7); // Runge-Kutta Cash-Karp tolerance (whether its absolute or relative is controlled by 'err')
    params->set_n(4); // Dommermuth initialization
    params->set_ta(0.0); // Dommermuth initialization

    // Physical constants
    params->set_grav(9.81); // Gravity
    params->set_depth(35.0); // Water depth

    // Irregular waves
    params->set_gamma(3.3); // JONSWAP shape parameter (used for initialization)
    params->set_beta(0.785398); // Directional spreading (Dysthe) parameter
    params->set_tp_real(10.0); // JONSWAP spectrum period (used for initialization)
    params->set_hs_real(4.5); // Significant wave height of the JONSWAP spectrum (used for initialization)
    ...
    // Horizontal domain
    params->set_xlen(10.0); // Number of wavelengths (only used if i_case=8x) or peak wavelengths (i_case=3x) in the domain along X-axis
    params->set_ylen(2.0); // Number of wavelengths (only used if i_case=8x) or peak wavelengths (i_case=3x) in the domain along Y-axis

    params->set_err("abs"); // Type of error used for the temporal integration scheme (abs for absolute, rel for relative
...
{
    public:
        Impl(const std::string& hos_server_address)
        : ctx(1)
        , socket(ctx, ZMQ_REQ)
        {
            socket.connect(hos_server_address);
            ...
            message.set_allocated_param(get_params());
...
HOS::HOS(const double ,
         ...
         const ssc::kinematics::PointMatrixPtr& output_mesh_) :
                SurfaceElevationInterface(output_mesh_, output_mesh_size_), pimpl(new Impl("tcp://hos-server:5550"))
{
