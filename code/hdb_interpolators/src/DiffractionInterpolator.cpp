...
DiffractionInterpolator::DiffractionInterpolator(
...
std::vector<std::vector<double> > DiffractionInterpolator::get_modules_cartesian(
...
std::vector<std::vector<double> > DiffractionInterpolator::get_phases_cartesian(
    const size_t k //!< Axis index (0 for Fx, 1 for Fy, 2 for Fz, 3 for Mx, 4 for My and 5 for Mz)
    )
{
...
double DiffractionInterpolator::interpolate_module(
    const size_t axis,
    const double Tp,
    double beta)
{
