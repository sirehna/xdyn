 ...
 *  \ingroup core
 *  \section ex1 Example
 *  \snippet core/unit_tests/src/ControllableForceModelTest.cpp ControllableForceModelTest example
 *  \section ex2 Expected output
 *  \snippet core/unit_tests/src/ControllableForceModelTest.cpp ControllableForceModelTest expected output
 */
class ControllableForceModel
{
    public:
        ...
        static ControllableForceParser build_parser()
        {
            auto parser = [](
                ...
                const EnvironmentAndFrames& env)
                -> boost::optional<ControllableForcePtr>
                {
                    boost::optional<ControllableForcePtr> ret;
                    if (yaml.model == ControllableForceType::model_name())
                    {
                        const std::string context = "Invalid input data for model '" +
                                                    ControllableForceType::model_name() + "'.";
                        try
                        {
                        ...
                        catch (const InvalidInputException& exception)
                        {
                            THROW(__PRETTY_FUNCTION__,
                                    ...
                                    context << std::endl
                                            ...
                                            << std::endl
                                            << "Model containing error is defined line "
                                            << yaml.index_of_first_line_in_global_yaml
                                            << " of the YAML file."
                                            << std::endl);
                        }
    ...
    private:
        ...
        std::map<std::string,double> get_commands(
            ssc::data_source::DataSource& command_listener,
            const double t
            ) const;
