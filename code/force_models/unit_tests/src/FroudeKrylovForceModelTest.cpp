...
TEST_F(FroudeKrylovForceModelTest, validate_formula_against_sos_stab)
{
    ...
    const double F1 = rho*g*dS*Hs/2*exp(-k*(z-wave.elevation_vectorized(x,y,t).at(0)))*cos(omega0*t-k*(x.at(0)*cos(psi)+y.at(0)*sin(psi))+phi);
    x.at(0)=0.1; y.at(0)=0; z = 0.2;
    const double F2 = rho*g*dS*Hs/2*exp(-k*(z-wave.elevation_vectorized(x,y,t).at(0)))*cos(omega0*t-k*(x.at(0)*cos(psi)+y.at(0)*sin(psi))+phi);
    x.at(0)=0; y.at(0)=-0.1; z = 0.2;
    const double F3 = rho*g*dS*Hs/2*exp(-k*(z-wave.elevation_vectorized(x,y,t).at(0)))*cos(omega0*t-k*(x.at(0)*cos(psi)+y.at(0)*sin(psi))+phi);
    x.at(0)=0; y.at(0)=0.1; z = 0.2;
    const double F4 = rho*g*dS*Hs/2*exp(-k*(z-wave.elevation_vectorized(x,y,t).at(0)))*cos(omega0*t-k*(x.at(0)*cos(psi)+y.at(0)*sin(psi))+phi);
    x.at(0)=0; y.at(0)=0; z = 0.1;
    const double F5 = rho*g*dS*Hs/2*exp(-k*(z-wave.elevation_vectorized(x,y,t).at(0)))*cos(omega0*t-k*(x.at(0)*cos(psi)+y.at(0)*sin(psi))+phi);
    x.at(0)=0; y.at(0)=0; z = 0.3;
    const double F6 = rho*g*dS*Hs/2*exp(-k*(z-wave.elevation_vectorized(x,y,t).at(0)))*cos(omega0*t-k*(x.at(0)*cos(psi)+y.at(0)*sin(psi))+phi);
