...
double OutputTransformer::compute_kinetic_energy(const size_t i, const StateType& x) const
{
    ...
    const auto IV = bodies.at(i)->get_states().solid_body_inertia->operator*(V);
...
double OutputTransformer::compute_potential_energy(const size_t i, const StateType& x) const
{
    ...
    {
        const double ep = (*that_force)->potential_energy(bodies.at(i)->get_states(),std::vector<double>(x.begin()+(long)i*13,x.begin()+(long)(i+1)*13-1));
        Ep += ep;
    }
    return Ep;
}
...
void OutputTransformer::fill_energy(std::map<std::string,double>& out, const size_t i, const StateType& res) const
{
    ...
    out[std::string("Ec(")+bodies.at(i)->get_name()+")"] = Ec;
    out[std::string("Ep(")+bodies.at(i)->get_name()+")"] = Ep;
    out[std::string("Em(")+bodies.at(i)->get_name()+")"] = Ec+Ep;
}
