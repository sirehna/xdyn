...
docker run $TERMINAL --security-opt seccomp=unconfined --rm -u $( id -u $USER ):$( id -g $USER ) -v $(pwd)/build_debian:/build -w /build -v $(pwd):/opt/share -t build-xdyn-debian ./run_all_tests `echo $*`
